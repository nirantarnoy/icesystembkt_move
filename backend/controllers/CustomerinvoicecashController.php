 ...
 */
class CustomerinvoicecashController extends Controller
{
...
//        $model = $sql_query->queryAll();
        ...
        $dataProvider->query->andFilterWhere(['customer_invoice.status' => 0,'invoice_type'=>2]);
...
//        $pre_date = date('Y-m-d', strtotime(date('Y-m-d') . "-3 month"));
        ...
        if ($cus_id) {
          ...
          //  $sql .= " AND (t1.payment_status !=1 OR ISNULL(t1.payment_status))";
            $sql .= " AND t1.payment_method_id = 1";
            $sql .= " AND t1.status != 3";
//            $sql .= " AND year(t1.order_date)>=2022";
...
//                    $html .= '<td style="text-align: center"><input type="file" class="form-control"></td>';
    ...
    public function actionPrintinvoice($id)
    {
        if ($id) {
            ...
            return $this->redirect('customerinvoicecash/index');
        }
