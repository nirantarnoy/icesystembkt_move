...
class Customerinvoice extends \common\models\CustomerInvoice
{
    ...
    {
        $model = Customerinvoice::find()->where(['company_id'=>$company_id,'branch_id'=>$branch_id])->andFilterWhere(['date(trans_date)' => date('Y-m-d')])->MAX('journal_no');
    ...
    public static function getInvAmount($id){
        $model = CustomerInvoiceLine::find()->where(['customer_invoice_id'=>$id])->sum('amount');
        return $model;
    }
