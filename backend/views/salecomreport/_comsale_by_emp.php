    ...
    <table class="table-title" id="table-data" style="width: 100%">
        ...
        <?php if ($find_emp_id != null): ?>
            <?php for ($k = 0; $k <= count($find_emp_id) - 1; $k++): ?>
                <?php $model_order_mobile = \common\models\Orders::find()->select(['order_channel_id','car_ref_id'])->where(['BETWEEN', 'order_date', date('Y-m-d H:i', strtotime($from_date)), date('Y-m-d H:i', strtotime($to_date))])
                    ->andFilterWhere(['company_id' => $company_id, 'branch_id' => $branch_id])
                    ->andFilterWhere(['sale_from_mobile' => 1])
...
//            }
                    ...
                    for ($m = 0; $m <= count($order_date) - 1; $m++):
                        ...
                        $order_data = null;
                        if (substr($route_name, 0, 2) == 'CJ') {
                            $order_data = getOrderlineCJ($value->order_channel_id, $order_date[$m]['order_date'], $company_id, $branch_id, $find_emp_id[$k]);
                        } else {
                            $order_data = getOrderline($value->order_channel_id, $order_date[$m]['order_date'], $company_id, $branch_id, $find_emp_id[$k]);
                        }
...
//                                $line_com = $order_data[0]['total_qty'] * $com_rate;
                            ...
                            $order_data_p2 = getOrderlineP2($value->order_channel_id, $order_date[$m]['order_date'], $company_id, $branch_id, $find_emp_id[$k]);
...
function getOrderlineP2($route_id, $f_date, $company_id, $branch_id, $emp_id)
{
    $data = [];
    if ($route_id != null) {
        $sql = "SELECT date(t2.order_date) as order_date, SUM((t2.line_qty_cash + t2.line_qty_credit)/t4.nw) as total_qty, SUM(t2.line_total) as total_amt, t3.emp_1,t3.emp_2
