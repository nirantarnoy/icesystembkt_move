    ...
    <table class="table-title" id="table-data" style="width: 100%">
        ...
        <?php foreach ($model_order_mobile as $value): ?>
            ...
            if (substr($route_name, 0, 2) == 'CJ') {
                if($route_emp_count == 1){
                    $com_rate = 1.5;
                }else if( $route_emp_count ==2){
                    $com_rate = 0.5;
                }

                $route_total = getRouteSumCJ($value->order_channel_id, $from_date, $to_date, $company_id, $branch_id);
            } else {
                $route_total = getRouteSum($value->order_channel_id, $from_date, $to_date, $company_id, $branch_id);
            }


            $route_com_total = getRouteComSum($value->order_channel_id, $from_date, $to_date, $company_id, $branch_id, $com_rate, $route_emp_count);
            $route_com_cal_sum = 0;
            $route_sp_cal_cum = 0;
            $route_com_sp_sum = 0;
            $route_com_cal_sum = $route_total[0]['total_qty'] * $com_rate;
            if ($route_com_total != null) {
                $route_sp_cal_sum = $route_com_total[0]['special'];
            }
            $route_com_sp_sum = $route_com_cal_sum + $route_sp_cal_sum;


            ?>
            <tr>
                <td style="text-align: left;border-top: 1px solid gray;border-bottom: 1px solid gray;border-right: 1px solid gray;border-left: 1px solid gray"><?= $route_name ?></td>
                ...
                <td style="text-align: right;border-top: 1px solid gray;border-bottom: 1px solid gray;border-right: 1px solid gray"><?= number_format($route_com_cal_sum, 2) ?></td>
                <td style="text-align: right;border-top: 1px solid gray;border-bottom: 1px solid gray;border-right: 1px solid gray"><?= number_format($route_sp_cal_sum) ?></td>
                <td style="text-align: right;border-top: 1px solid gray;border-bottom: 1px solid gray;border-right: 1px solid gray"><?= number_format($route_com_sp_sum, 2) ?></td>
            </tr>
            <?php
            $order_data = null;
            if (substr($route_name, 0, 2) == 'CJ') {
                $order_data = getOrderCJ($value->order_channel_id, $from_date, $to_date, $company_id, $branch_id);
            } else {
            ...
            for ($m = 0; $m <= count($order_data) - 1; $m++):
                ...
                $line_com = $order_data[$m]['total_qty'] * $com_rate;
                ...
                <tr style="background-color: #d0e9c6">
                    <td colspan="4"
                        ...
                        <span> </span> <?= $order_data[$m]['order_date'] ?></td>
        ...
        <tr>
            ...
            <td style="text-align: right;border-top: 1px solid gray;border-bottom: 1px solid gray;border-right: 1px solid gray">
                <b><?= number_format($total_qty_all) ?></b></td>
            <td style="text-align: right;border-top: 1px solid gray;border-bottom: 1px solid gray;border-right: 1px solid gray">
                <b><?= number_format($total_amt_all) ?></b></td>
            <td style="text-align: right;border-top: 1px solid gray;border-bottom: 1px solid gray;border-right: 1px solid gray">
...
function getOrder($route_id, $f_date, $t_date, $company_id, $branch_id)
{
    ...
    $data = [];
    if ($route_id != null) {
//        $sql = "SELECT date(t2.order_date) as order_date, SUM(t3.qty) as total_qty, SUM(t3.line_total) as total_amt
...
function getRouteComSum($route_id, $from_date, $to_date, $company_id, $branch_id, $com_rate, $route_emp_count)
{
    ...
    $total_com_sum_all = 0;
    $total_special_all = 0;
    $order_data = getOrder($route_id, $from_date, $to_date, $company_id, $branch_id);
    for ($m = 0; $m <= count($order_data) - 1; $m++) {
        $line_special = $order_data[$m]['total_amt'] >= 3500 && $route_emp_count == 1 ? 30 : 0;
        $total_special_all = $total_special_all + $line_special;
    }
