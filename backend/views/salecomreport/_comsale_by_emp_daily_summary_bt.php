...
<form action="<?= \yii\helpers\Url::to(['salecomreport/comdailycalbkt'], true) ?>" id="form-cal" method="post">
    <div class="row">
        ...
        <div class="col-lg-3" style="text-align: right;">
            <a href="<?= \yii\helpers\Url::to(['salecomreport/index2'], true) ?>" class="btn btn-info" target="_self">
                คำนวนย้อนหลัง</a>
        </div>
    </div>
</form>
<br/>
<form action="<?= \yii\helpers\Url::to(['salecomreport/index3'], true) ?>" method="post" id="form-search">
...
<div id="div1">
    <table class="table-header" width="100%">
        <tr>
            <td style="text-align: center; font-size: 20px; font-weight: bold">รายงานค่าคอมมิชชั่นแยกพนักงาน</td>
        </tr>
    </table>
    <br>
    <table class="table-header" width="100%">
...
//            }
                    ...
                    <tr>
                        ...
                        <!--                <td style="text-align: left;border-top: 1px solid gray;border-bottom: 1px solid gray;border-right: 1px solid gray"></td>-->
                        <td colspan="5" style="border-right: 1px solid gray"></td>
                    ...
                    <?php foreach ($model_order_mobile as $value): ?>
                        ...
                        $line_new_total = 0;
                        if ($find_emp_id[$k] == $value->emp_1) {
                            $line_new_total = $value->line_com_amt;
                            $is_driver = '(คนขับ)';
                        } else if ($find_emp_id[$k] == $value->emp_2) {
                            $line_new_total = $value->line_com_amt_2;
                            $is_emp2 = 1;
                            $is_driver = '(ผู้ติดตาม)';
                        }
                        ...
                        $total_com_all = ($total_com_all + $line_new_total);
                        $total_com_all_2 = ($total_com_all_2 + $line_com_total_2);
                        $total_special_all = ($total_special_all + $line_special);
                        $total_com_sum_all = $total_com_sum_all + ($line_new_total + $line_special);
                        ?>
                        <tr style="background-color: #d0e9c6">
                            ...
                            <td style="text-align: left;border-top: 1px solid gray;border-bottom: 1px solid gray;border-right: 1px solid gray"><?= \backend\models\Employee::findName2($find_emp_id[$k]).' '.$is_driver ?></td>
                            ...
                            <td style="text-align: right;border-top: 1px solid gray;border-bottom: 1px solid gray;border-right: 1px solid gray;"><?= $is_emp2 == 1 ? '<b style="color: red">' : '' ?><?= number_format($line_new_total, 2) ?><?= $is_emp2 == 1 ? '</b>' : '' ?></td>
                            ...
                            <td style="text-align: right;border-top: 1px solid gray;border-bottom: 1px solid gray;border-right: 1px solid gray"><?= number_format(($line_new_total + $line_special), 2) ?></td>
                        </tr>
