...
<form action="<?=\yii\helpers\Url::to(['salecomreport/comdailycalprev'],true)?>" id="form-cal" method="post">
    <div class="row">
    ...
    <table class="table-title" id="table-data" style="width: 100%">
        ...
        <?php if ($find_emp_id != null): ?>
            <?php for ($k = 0; $k <= count($find_emp_id) - 1; $k++): ?>
                ...
                $model_order_mobile = \common\models\ComDailyCal::find()->select(['trans_date', 'route_id', 'car_id', 'emp_1', 'emp_2', 'line_total_amt', 'total_qty', 'total_amt', 'line_com_amt', 'line_com_special_amt'])->where(['BETWEEN', 'trans_date', date('Y-m-d H:i:s', strtotime($from_date)), date('Y-m-d H:i:s', strtotime($to_date))])
                    ->andFilterWhere(['company_id' => $company_id, 'branch_id' => $branch_id])
...
//            }
                    ...
                    $group_total_com = ($value->line_com_amt);
                    ...
                    $total_qty_all = ($total_qty_all + $group_total_qty);
                    $total_amt_all = ($total_amt_all + $group_total_amt);
                    ...
                    $total_com_sum_all = $total_com_sum_all + ($line_com_total + $line_special);
                    ?>
                    <tr style="background-color: #d0e9c6">
                        ...
                        <td style="text-align: right;border-top: 1px solid gray;border-bottom: 1px solid gray;border-right: 1px solid gray"><?= number_format(($line_com_total + $line_special), 2) ?></td>
                    </tr>
...
<br/>
<table width="100%" class="table-title" style="display: none;">
