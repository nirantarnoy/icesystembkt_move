...
$sql .= " AND t2.sale_from_mobile=1";
$sql .= " AND t2.status = 100";
$sql .= " AND t1.price > 0";
...
//}
?>
<form action="<?= \yii\helpers\Url::to(['saleqtysumbyprice/index2'], true) ?>" method="post" id="form-search">
                ...
                if ($data) {
                    for ($ix = 0; $ix < count($data); $ix++) {
                        ...
                        <tr>
                            ...
                            <?php
                            $product_price_data = getLineprice($data[$ix]['product_id'], (float)$data[$ix]['price'], $days, $column_total, $fdate, $tdate, $is_invoice_req);
                            // print_r($product_price_data);return;
                            ?>
                            <?php $line_total = 0; ?>
...
<?php
function getLineprice($product_id, $price, $days, $column_total, $fdate, $tdate, $is_invoice_req)
{
    $data = [];
    if ($product_id != null && $price != null && $days != null) {
        $sql = "SELECT day(t2.order_date) as day,sum(t1.qty) as qty";
        $sql .= " FROM order_line as t1 INNER JOIN orders as t2 ON t1.order_id = t2.id left join customer as t3 on t1.customer_id=t3.id left join delivery_route as t4 on t2.order_channel_id = t4.id";
        $sql .= " WHERE date(t2.order_date) >=" . "'" . date('Y-m-d', strtotime($fdate)) . "'" . " ";
        $sql .= " AND date(t2.order_date) <=" . "'" . date('Y-m-d', strtotime($tdate)) . "'" . " ";
        $sql .= " AND t2.sale_from_mobile=1";
        $sql .= " AND t2.status = 100";
        $sql .= " AND t1.status <> 500";
...
//        }
        ...
        if ($modelx) {
            for ($i = 0; $i < ($days + 1); $i++) {
                $day_has = 0;
                for ($ix = 0; $ix < count($modelx); $ix++) {
                    if ((int)$column_total[$i]['day'] == (int)$modelx[$ix]['day']) {
