...
if($from_date !='' && $to_date != ''){
    if ($find_route_id != null) {
        $model_line = \common\models\DeliveryRoute::find()->select(['id'])->where(['id'=>$find_route_id,'status'=>1])->orderBy(['id'=>SORT_ASC])->all();

    } else {
        $model_line = \common\models\DeliveryRoute::find()->select(['id'])->where(['status'=>1])->orderBy(['id'=>SORT_ASC])->all();
...
<div class="row">
    <div class="col-lg-12">
        <form action="<?= \yii\helpers\Url::to(['adminreportcarissue/index2'], true) ?>" method="post" id="form-search">
...
<div id="div1">
    <table class="table-header" width="100%">
        <tr>
            <td style="text-align: center; font-size: 20px; font-weight: bold">รายงานสรุปสินค้าเบิกสายส่ง</td>
        </tr>
    </table>
    <br>
    <table class="table-header" width="100%">
    ...
    $modelx = \common\models\Product::find()->select(['id'])->where(['status'=>1])->orderBy(['item_pos_seq' => SORT_ASC])->all();

    if ($modelx) {
        foreach ($modelx as $valuexx) {
            if (!in_array($valuexx->id, $product_header)) {
                array_push($product_header, $valuexx->id);
            }
        }
    }
    ...
    <table id="table-data">
        ...
        <?php foreach ($model_line as $value): ?>
            ...
            $car_data = getCardata($value->id, $from_date);
            ?>
            <tr>
                <td style="text-align: left;padding: 8px;border: 1px solid grey"><?= \backend\models\Deliveryroute::findName($value->id) ?></td>
                ...
                <?php for ($x = 0; $x <= count($product_header) - 1; $x++): ?>
                    <?php
                    $return_line_qty = getCarissueSum($value->id, $product_header[$x], $from_date, $to_date);
            ...
            <?php for ($xx = 0; $xx <= $cnt_day; $xx++): ?>
                ...
                <tr>
                    ...
                    <?php for ($x = 0; $x <= count($product_header) - 1; $x++): ?>
                        ...
                        $return_line_route_qty = getOrder($product_header[$x], $loop_date,$value->id);
...
function getReturncarQty($route_id, $product_id, $from_date, $to_date)
{
    ...
    if ($route_id && $product_id) {
        $model_qty = \common\models\StockTrans::find()->where(['BETWEEN', 'date(trans_date)', date('Y-m-d', strtotime($from_date)), date('Y-m-d', strtotime($to_date))])
            ->andFilterWhere(['trans_ref_id' => $route_id, 'product_id' => $product_id, 'activity_type_id' => [20]])->SUM('qty');
        if ($model_qty != null) {
            $data = ($model_qty);
        }
    }
...
function getCarissueSum($route_id, $product_id, $from_date, $to_date){
    $qty = 0;
    $sql = "SELECT  sum(t1.qty) as qty
              FROM order_line as t1 INNER JOIN orders as t2 ON t1.order_id = t2.id
             WHERE  date(t2.order_date) >=" . "'" . date('Y-m-d',strtotime($from_date)). "'" . " 
             AND  date(t2.order_date) <=" . "'" . date('Y-m-d',strtotime($to_date)). "'" . " 
...
function getReturncarQtyByroute($route_id, $product_id, $trans_date)
{
    ...
    if ($route_id && $product_id) {
        $model_qty = \common\models\StockTrans::find()->select(['qty'])->where(['date(trans_date)' => date_format($trans_date, "Y-m-d")])
            ->andFilterWhere(['trans_ref_id' => $route_id, 'product_id' => $product_id, 'activity_type_id' => [20]])->one();
        if ($model_qty != null) {
            $data = ($model_qty->qty);
        }
    }
...
function getOrder($product_id, $trans_date, $route_id)
{
    $data = [];
    $qty = 0;
    $sql = "SELECT  sum(t1.qty) as qty
              FROM order_line as t1 INNER JOIN orders as t2 ON t1.order_id = t2.id
             WHERE  date(t2.order_date) =" . "'" . date_format($trans_date,"Y-m-d"). "'" . " 
             AND t1.product_id=" . $product_id . " 
             AND t2.status <> 3
             AND t2.sale_channel_id = 2";
